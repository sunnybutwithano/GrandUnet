...
class GNNEarly(BaseGNN):
  ...
  def __init__(self, opt, dataset, device=torch.device('cpu'), num_features=None):
    from run_GNN import Print
    
    root = './data/CORA'
    dataset1 = torch_geometric.datasets.Planetoid(root, name='Cora')

    # lines = dict()
    # for xx in range(dataset1.x.shape[0]):
    #   lines.update({xx: list()})
    
    
    # for l in dataset1.edge_index.T:
      
    #   i, j = l
    #   lines[i.item()].append(j.item())
    # with open('cora.txt', 'w') as f:
    #   for line in lines.values():
        
    #     f.write(' '.join(['9'] + [str(len(line))] + [str(i) for i in line]) + '\n')

    # raise
    
    super(GNNEarly, self).__init__(opt, dataset, device, num_features)
  ...
  def forward(self, x, new_edge_index, return_hidden=False, pos_encoding=None):
    from run_GNN import Print 
    ...
    if self.training  and self.odeblock.nreg > 0:
      ...
      z = self.odeblock(x, new_edge_index)
